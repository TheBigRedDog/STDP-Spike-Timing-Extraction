...
def filter_APs(stim_results, stim_time, stim_voltage, sweep_num, stim_num):
    ...
    units long and have similar characteristics. This filtering allows us to catch
...
def calculate_AP_frequencies_changing_current(stim_results):
    ...
    num_APs_first_100ms = len([peak for peak in AP_indices if 100 <= peak <1100])
    num_APs_second_100ms = len([peak for peak in AP_indices if 1100 <= peak <= 2100])
...
def extract_pre_post_timepoints(sweep_num, sweep_voltage, sweep_time, stim_num, injection_times, stim_frequencies, stim_EPSP_offset, stim_offset=100):
    # stim offset is 100 to put start of recording 10 ms before each stimulation (x interval for abf.sweepX is 0.1 ms, hence 100)
    ...
    AP_Threshold = np.mean(AP_results[0]['peak_voltage'])-10
    ...
    if hz1 != hz2:
        ...
        hz_1_indices = np.arange(100+stim_EPSP_offset, 1100+stim_EPSP_offset, hz1_step) #offset by delay between presynaptic stimulation and resulting EPSP
        hz_2_indices = np.arange(1100+stim_EPSP_offset, 2100+stim_EPSP_offset, hz2_step)
        pre_stim_indices = np.concatenate((hz_1_indices, hz_2_indices), axis=0)
        artifact_1_indices = np.arange(100, 1100, hz1_step) #offset by delay between presynaptic stimulation and resulting EPSP
        artifact_2_indices = np.arange(1100, 2100, hz2_step)
        stimulation_artifact_indices = np.concatenate((artifact_1_indices, artifact_2_indices), axis=0)
    else:
        hz_step = 1/hz1 * 10000
        pre_stim_indices = np.arange(100+stim_EPSP_offset, 2100+stim_EPSP_offset, hz_step) #offset by delay between presynaptic stimulation and resulting EPSP
        stimulation_artifact_indices = np.arange(100, 2100, hz_step)
...
for abf_file in abf_files:
    ...
    for Sweep_Number in range(len(abf.sweepList)):
        ...
        print(len(sweep_time))
        print(len(sweep_voltage))
        print(len(sweep_command))
        ...
        print(current_injection_times)
        current_injection_times = [(current_injection_times[0][0], current_injection_times[-1][-1])]
        print(current_injection_times)
